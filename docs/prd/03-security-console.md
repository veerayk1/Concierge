...
> Added from GAP-ANALYSIS-FINAL.md gaps 3.1, 3.2, 3.3, 3.4, 3.6

### A1. Fire Event Type — Specialized Checklist Fields (Gap 3.1, Critical)

The Fire event type MUST include the following specialized fields beyond the standard event fields. These are safety-critical and observed in Platform 1.

#### Fire Event Additional Fields

| #   | Field                          | Type        | Required | Default | Validation                              | Description                                                         |
| --- | ------------------------------ | ----------- | -------- | ------- | --------------------------------------- | ------------------------------------------------------------------- |
| 1   | time_alarm_went_off            | DateTime    | Yes      | Now     | Must be <= now                          | Time the fire alarm activated                                       |
| 2   | alarm_location                 | String(200) | Yes      | —       | Not blank                               | Physical location of the alarm (e.g., "Floor 12, Unit 1205")        |
| 3   | alarm_type                     | String(100) | Yes      | —       | Not blank                               | Type of alarm (e.g., "Smoke Detector", "Pull Station", "Sprinkler") |
| 4   | time_called_fire_department    | DateTime    | Yes      | —       | Must be >= time_alarm_went_off          | When fire department was called                                     |
| 5   | time_first_announcement        | DateTime    | Yes      | —       | Must be >= time_alarm_went_off          | When first PA announcement was made                                 |
| 6   | time_second_announcement       | DateTime    | No       | —       | Must be >= time_first_announcement      | When second PA announcement was made                                |
| 7   | time_third_announcement        | DateTime    | No       | —       | Must be >= time_second_announcement     | When third PA announcement was made                                 |
| 8   | time_fire_department_arrived   | DateTime    | Yes      | —       | Must be >= time_called_fire_department  | When fire department arrived on scene                               |
| 9   | time_fire_department_all_clear | DateTime    | No       | —       | Must be >= time_fire_department_arrived | When fire department gave all-clear                                 |
| 10  | time_fire_department_departed  | DateTime    | Yes      | —       | Must be >= time_fire_department_arrived | When fire department left the building                              |

#### Fire Preparation Checklist (3 items, all boolean)

| #   | Checklist Item                  | Default | Description                                                      |
| --- | ------------------------------- | ------- | ---------------------------------------------------------------- |
| 1   | fire_safety_plan_ready          | false   | Fire Safety Plan retrieved and available                         |
| 2   | fire_department_keys_ready      | false   | Fire department keys retrieved and available                     |
| 3   | assistance_residents_list_ready | false   | List of residents requiring fire assistance is printed/available |

#### Elevator Response Checklist (configurable, default 4 items)

| #   | Checklist Item       | Default | Description                                 |
| --- | -------------------- | ------- | ------------------------------------------- |
| 1   | elevator_1_responded | false   | Elevator 1 recalled to lobby (if available) |
| 2   | elevator_2_responded | false   | Elevator 2 recalled to lobby (if available) |
| 3   | elevator_3_responded | false   | Elevator 3 recalled to lobby (if available) |
| 4   | elevator_4_responded | false   | Elevator 4 recalled to lobby (if available) |

Number of elevator checklist items is configurable per property in Settings (Tab: Property Setup). Properties set their elevator count during onboarding.

#### Reset Electronic Devices Checklist (7 items, all boolean)

| #   | Checklist Item       | Default | Description                            |
| --- | -------------------- | ------- | -------------------------------------- |
| 1   | reset_pull_station   | false   | Fire pull station reset                |
| 2   | reset_smoke_detector | false   | Smoke detector reset                   |
| 3   | reset_heat_detector  | false   | Heat detector reset                    |
| 4   | reset_sprinkler_head | false   | Sprinkler head inspected/reset         |
| 5   | reset_fire_panel     | false   | Fire panel reset to normal             |
| 6   | reset_mag_locks      | false   | Magnetic door locks reset              |
| 7   | reset_elevators      | false   | Elevators returned to normal operation |

#### Fire Event UI

The fire event creation form uses a **step-by-step guided layout** (not a single long form):

1. Step 1: Alarm details (time, location, type)
2. Step 2: Emergency response (FD call, announcements)
3. Step 3: Preparation checklist (auto-populated, staff checks off)
4. Step 4: Elevator response checklist
5. Step 5: Resolution (FD arrival, all-clear, departure)
6. Step 6: Reset devices checklist
7. Step 7: Additional notes and attachments

Each step auto-saves. Staff can navigate back/forward. The form is designed for high-stress situations -- large touch targets, clear labels, minimal cognitive load.

### A2. Noise Event Type — Specialized Fields (Gap 3.2, Critical)

The Noise event type MUST include the following specialized fields. These are observed in Platform 1 and critical for bylaw enforcement documentation.

#### Nature of Complaint (Multi-select checkboxes, at least one required)

| #   | Complaint Type   | Description                                    |
| --- | ---------------- | ---------------------------------------------- |
| 1   | Drop on Floor    | Objects dropped on floor causing impact noise  |
| 2   | Loud Music       | Music played at excessive volume               |
| 3   | Smoking Hallways | Smoking in common hallways                     |
| 4   | Smoking in Suite | Smoking inside unit (smell traveling)          |
| 5   | Hallway Noise    | General noise in hallways                      |
| 6   | Piano Playing    | Musical instrument noise                       |
| 7   | Dog Barking      | Pet noise                                      |
| 8   | Cooking Odors    | Strong cooking smells affecting neighbors      |
| 9   | Children Playing | Noise from children's activities               |
| 10  | Walking/Banging  | Footstep or banging noise                      |
| 11  | Party            | Party noise                                    |
| 12  | Talking          | Loud conversation                              |
| 13  | Construction     | Unauthorized or after-hours construction noise |
| 14  | Other            | Free-text description if "Other" is selected   |

These categories are admin-configurable in Settings. The 14 listed above are the system defaults.

#### Investigation Fields

| #   | Field                              | Type    | Required | Options                                             |
| --- | ---------------------------------- | ------- | -------- | --------------------------------------------------- |
| 1   | noise_noticeable_complainant_floor | Enum    | Yes      | "Clearly audible", "Faintly audible", "Not audible" |
| 2   | noise_noticeable_suspect_floor     | Enum    | Yes      | "Clearly audible", "Faintly audible", "Not audible" |
| 3   | noise_duration_suspect_floor       | Enum    | Yes      | "Continuous", "Intermittent", "Brief"               |
| 4   | noise_volume_suspect_floor         | Enum    | Yes      | "Very loud", "Loud", "Moderate", "Quiet"            |
| 5   | length_of_time_verified_minutes    | Integer | No       | 0-999. How long security verified the noise         |

#### Suspect Contact Method (Multi-select checkboxes)

| #   | Method         | Description                      |
| --- | -------------- | -------------------------------- |
| 1   | Home Phone     | Called suspect's home phone      |
| 2   | Work Phone     | Called suspect's work phone      |
| 3   | Cell Phone     | Called suspect's cell phone      |
| 4   | Door Visit     | Visited suspect's unit in person |
| 5   | Written Notice | Left a written notice            |

#### Resolution / Counseling (Multi-select checkboxes)

| #   | Item                    | Description                                    |
| --- | ----------------------- | ---------------------------------------------- |
| 1   | Suspect counseled       | Suspect was verbally counseled about the noise |
| 2   | Written warning issued  | Formal written warning delivered               |
| 3   | Noise stopped           | Noise ceased after intervention                |
| 4   | Escalated to management | Issue escalated to property management         |

### A3. Authorized Entry as Standalone Event Type (Gap 3.3, High)

Add "Authorized Entry" as a v1 event type (in addition to the existing 10). This tracks when staff enter a resident's unit with authorization.

#### Authorized Entry Fields

| #   | Field                | Type        | Required   | Description                                                                        |
| --- | -------------------- | ----------- | ---------- | ---------------------------------------------------------------------------------- |
| 1   | unit_id              | FK -> Unit  | Yes        | Unit being entered                                                                 |
| 2   | authorized_by        | String(200) | Yes        | Who gave authorization (resident name or "Standing Authorization")                 |
| 3   | authorization_method | Enum        | Yes        | "Verbal (phone)", "Written (email)", "In-person", "Standing authorization on file" |
| 4   | reason               | String(500) | Yes        | Reason for entry (e.g., "Maintenance repair", "Emergency", "Inspection")           |
| 5   | entry_time           | DateTime    | Yes (auto) | When staff entered the unit                                                        |
| 6   | exit_time            | DateTime    | No         | When staff exited the unit (nullable until completed)                              |
| 7   | staff_member_id      | FK -> Staff | Yes (auto) | Staff member who entered                                                           |
| 8   | accompanying_persons | String(500) | No         | Names of vendor/contractor who accompanied                                         |

This event type creates an audit trail for liability protection and is critical for condo environments where unit entry is legally sensitive.

### A4. Security Patrol Entry Type (Gap 3.4, Medium)

Add "Security Patrol" as a v1 event type for tracking guard patrol rounds.

| #   | Field                 | Type        | Required | Description                                     |
| --- | --------------------- | ----------- | -------- | ----------------------------------------------- |
| 1   | patrol_route          | String(200) | Yes      | Name of patrol route (configurable in Settings) |
| 2   | start_time            | DateTime    | Yes      | Patrol start time                               |
| 3   | end_time              | DateTime    | No       | Patrol end time                                 |
| 4   | checkpoints_completed | JSON        | No       | Array of checkpoint IDs with timestamps         |
| 5   | observations          | Text        | No       | Notes from the patrol                           |
| 6   | incidents_found       | Boolean     | No       | Whether any incidents were discovered           |

### A5. Pass-On Log vs Shift Log Distinction (Gap 3.6, Medium)

Clarify that "Pass-On Log" and "Shift Log" are distinct concepts:

- **Shift Log**: The record of a security shift (who worked, start/end times, summary of events during shift). Created once per shift.
- **Pass-On Log**: Individual notes left by one shift for the next (e.g., "Unit 302 has a plumber coming tomorrow at 9am", "Package for 1205 is behind the desk"). Multiple pass-on log entries per shift.

Both should be event types in the unified event model. The existing Shift Log specification covers both concepts but should be explicitly split into two event types for clarity.

---

_End of document._
_Total entry types: 10 (v1) + 4 (addendum) + 6 (v2) + 4 (v3+)_
_Total data model entities: 18 + 4 (addendum)_
_Total AI capabilities: 12_
_Total API endpoints: 25+_
_Total notification types: 13_
