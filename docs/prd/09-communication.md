...
| Italic         | _I_               | Toggle italic on selected text                                                              |
...
> Added from GAP-ANALYSIS-FINAL.md gaps 9.1, 9.3

### A1. Digital Signage / Lobby Display Channel (Gap 9.1, High)

Both Platform 1 ("virtual concierge board") and Platform 2 ("Public Display" with full configuration) support lobby screen distribution. CLAUDE.md lists this as v3+ but both competitors have it. Promote to v2 scope.

#### Lobby Display Channel Specification

| Aspect                | Specification                                                                                                                           |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **Version**           | v2 (promoted from v3+)                                                                                                                  |
| **Channel name**      | "Lobby Display"                                                                                                                         |
| **Behavior**          | When creating an announcement, staff can check "Lobby Display" as a distribution channel                                                |
| **Display format**    | Simplified view: title, body (truncated), posted date. No interactive elements.                                                         |
| **Configuration**     | Settings > Communication > Lobby Display: enable/disable, display duration per announcement (seconds), rotation order                   |
| **Public URL**        | Each property gets a `/display/{property-slug}` URL that renders a full-screen, auto-rotating announcement view suitable for TV/monitor |
| **No authentication** | The display URL does not require login (it is a public, read-only view of announcements marked for lobby display)                       |
| **Auto-refresh**      | Display page auto-refreshes every 5 minutes to pick up new announcements                                                                |

### A2. Email Delivery Failure Investigation UI (Gap 9.3, High)

Platform 1 has "Email Failures" and "Mailing List" buttons on each announcement showing detailed delivery failure data. Concierge must match this.

#### Per-Announcement Delivery Report

When staff clicks "Delivery Report" on an announcement, a modal shows:

| Column          | Description                                                                               |
| --------------- | ----------------------------------------------------------------------------------------- |
| Recipient Email | Target email address                                                                      |
| Recipient Name  | Resident/staff name                                                                       |
| Unit #          | Unit number (if resident)                                                                 |
| Channel         | Email, SMS, Push                                                                          |
| Status          | "Delivered", "Bounced", "Failed", "Pending"                                               |
| Error Reason    | Specific error (e.g., "Invalid email", "Mailbox full", "Spam filter", "No email on file") |
| Timestamp       | When delivery was attempted                                                               |

#### Missing Contact Information Alert

On the Communication dashboard, display a proactive alert:

- "X staff members missing email addresses"
- "Y residents missing email addresses"

Clicking either count navigates to a filtered user list showing only users with missing contact info, enabling admins to fix data quality issues.

---

_Last updated: 2026-03-17_
_Module: Communication_
_Dependencies: 01-Architecture, 02-Roles and Permissions, 19-AI Framework_
_Integration points: Twilio (SMS, Voice), SendGrid (Email), Firebase (Push), AI Gateway (all AI features)_
