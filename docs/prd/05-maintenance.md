...
## V2 Feature Expansions

### Equipment Tracking

Equipment tracking transforms reactive maintenance ("something broke, fix it") into proactive asset management ("this boiler is 8 years into a 10-year lifespan, budget for replacement").

#### Asset Master List

Every piece of building equipment is registered as an asset with the following fields:

| Field                     | Type            | Max Length | Required | Default | Validation                                                                                                                                       | Error Message                                         |
| ------------------------- | --------------- | ---------- | -------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------- |
| Asset Name                | Text            | 200 chars  | Yes      | --      | Non-empty                                                                                                                                        | "Asset name is required"                              |
| Category                  | Dropdown        | --         | Yes      | --      | Must select from configured categories (HVAC, Plumbing, Electrical, Elevator, Fire Safety, Pool/Spa, Gym, Common Area, Parking, Security, Other) | "Select a category"                                   |
| Manufacturer              | Text            | 200 chars  | No       | --      | --                                                                                                                                               | --                                                    |
| Model                     | Text            | 200 chars  | No       | --      | --                                                                                                                                               | --                                                    |
| Serial Number             | Text            | 100 chars  | No       | --      | Unique within property if provided                                                                                                               | "This serial number already exists for another asset" |
| Location                  | Dropdown + Text | 200 chars  | Yes      | --      | Dropdown: Unit number or Common Area name. Free-text field for specific location (e.g., "Mechanical Room B, north wall").                        | "Location is required"                                |
| Acquisition Date          | Date            | --         | No       | --      | Cannot be in the future                                                                                                                          | "Acquisition date cannot be in the future"            |
| Warranty Expiry           | Date            | --         | No       | --      | Must be after acquisition date                                                                                                                   | "Warranty expiry must be after acquisition date"      |
| Expected Lifespan (years) | Number          | --         | No       | --      | 1-100                                                                                                                                            | "Lifespan must be between 1 and 100 years"            |
| Replacement Cost          | Currency        | --         | No       | --      | Positive number, 2 decimal places                                                                                                                | "Enter a valid amount"                                |
| Condition                 | Dropdown        | --         | No       | "Good"  | Excellent, Good, Fair, Poor, End of Life                                                                                                         | "Select a condition"                                  |
| Photo                     | File upload     | 4 MB       | No       | --      | JPG, PNG, HEIC                                                                                                                                   | "Upload a JPG, PNG, or HEIC image under 4 MB"         |
| Notes                     | Textarea        | 2000 chars | No       | --      | --                                                                                                                                               | "Notes cannot exceed 2000 characters"                 |

#### Service History

Each asset's detail page includes a "Service History" tab that displays all linked maintenance requests in reverse chronological order. When creating a maintenance request, a "Link to Equipment" dropdown allows staff to associate the request with a specific asset. This creates a complete service record per asset over time.

| Column       | Description                                                |
| ------------ | ---------------------------------------------------------- |
| Date         | When the maintenance was performed                         |
| Request ID   | Link to the full maintenance request (e.g., SR-2026-00147) |
| Description  | Summary of work performed                                  |
| Performed By | Staff member or vendor name                                |
| Cost         | If tracked, the cost of the service                        |

#### QR/Barcode Labels

| Aspect           | Detail                                                                                                                                                                     |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Generation**   | Each asset automatically gets a unique QR code containing a URL: `concierge.com/equipment/[asset_id]`                                                                      |
| **Print**        | "Print Label" button on the asset detail page generates a printable label (thermal printer compatible) with: QR code, asset name, serial number, and property name.        |
| **Scan**         | Scanning the QR code with any device camera opens the asset profile page. If the user is authenticated, they see full details. If not, they are redirected to login first. |
| **Quick action** | Below the asset profile, a "Report Issue" button creates a new maintenance request pre-linked to this equipment.                                                           |

#### Depreciation Tracking

| Aspect            | Detail                                                                                                                                              |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Method**        | Straight-line depreciation: `(Replacement Cost - Salvage Value) / Expected Lifespan`                                                                |
| **Salvage value** | Default: 10% of replacement cost (configurable per asset)                                                                                           |
| **Display**       | Asset detail page shows: Original Cost, Current Book Value, Annual Depreciation, Years Remaining                                                    |
| **Report**        | "Equipment Depreciation Report" available in the Reports module (PRD 15): lists all assets with current book value, sorted by category, with totals |

#### Replacement Forecasting

| Aspect                     | Detail                                                                                                                                                                                                                      |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Calculation**            | Based on `Acquisition Date + Expected Lifespan`, adjusted by current `Condition` rating. Assets in "Poor" condition have their forecast moved forward by 20%. Assets in "End of Life" are flagged as "Replace Immediately." |
| **Dashboard widget**       | "Upcoming Replacements" card on the Property Manager dashboard: shows assets due for replacement in the next 12 months with estimated costs.                                                                                |
| **Budget planning report** | Exportable report showing replacement forecasts for the next 1, 3, and 5 years, grouped by category, with total estimated costs per year.                                                                                   |

---

### Inspection Checklists

Mobile-first inspection system that replaces paper checklists with digital forms, automatic follow-up actions, and compliance tracking.

#### Checklist Builder

Property Admins and Property Managers build inspection checklists using a drag-and-drop interface:

| Question Type  | Description                                 | Options                                                        |
| -------------- | ------------------------------------------- | -------------------------------------------------------------- |
| Yes / No       | Binary toggle                               | Default pass/fail mapping configurable                         |
| Pass / Fail    | Binary with explicit pass/fail semantics    | Auto-creates maintenance request on "Fail" (configurable)      |
| Dropdown       | Single-select from predefined options       | Admin defines options list                                     |
| Multi-Select   | Multiple selections from predefined options | Admin defines options list                                     |
| Text           | Free-text response                          | Max 1000 characters                                            |
| Number         | Numeric input                               | Min/max range configurable (e.g., temperature reading 15-30 C) |
| Photo Required | Requires a photo upload to proceed          | Minimum 1 photo, maximum 5                                     |
| Signature      | Signature capture pad                       | Required at end of inspection for sign-off                     |

#### Branching Logic

| Aspect                 | Detail                                                                                                                                                                                                                 |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Trigger**            | Any answer can trigger a branch. Configured per question in the checklist builder.                                                                                                                                     |
| **Actions on trigger** | Show follow-up questions (conditional section), auto-create a maintenance request with pre-filled category and description, send a notification to a specific role.                                                    |
| **Example**            | "Fire extinguisher pressure gauge in green zone?" → If "No" → show follow-up: "Describe the issue" (text) + "Take a photo" (photo required) + auto-create maintenance request tagged "Fire Safety" with high priority. |

#### Templates

Pre-built inspection templates that properties can adopt and customize:

| Template            | Frequency | Questions                                                                                         |
| ------------------- | --------- | ------------------------------------------------------------------------------------------------- |
| Fire Safety Monthly | Monthly   | 15 questions covering extinguishers, sprinklers, exit signs, fire doors, alarm panels             |
| HVAC Quarterly      | Quarterly | 20 questions covering filters, coils, condensate drains, thermostats, ductwork                    |
| Pool Daily          | Daily     | 12 questions covering chemical levels, equipment, cleanliness, safety equipment                   |
| Elevator Annual     | Annual    | 25 questions covering cab condition, door operation, emergency phone, certificates, pit condition |
| Common Area Weekly  | Weekly    | 10 questions covering lighting, cleanliness, signage, damage, safety hazards                      |
...
| **Offline capable**    | Inspection forms download to device. Responses saved locally. Synced when connectivity returns. Conflict resolution: device data wins (inspector was physically present).                                                                                                             |
| **GPS verification**   | On inspection start, device GPS coordinates are captured. If the inspector is more than 200 meters from the property address, a warning is shown: "You appear to be off-site. Continue anyway?" (with "Yes" and "No" options). GPS coordinates are stored with the inspection record. |
| **Photo geolocation**  | All photos taken during an inspection are tagged with GPS coordinates and timestamp. These are embedded in the photo metadata and displayed on the inspection report.                                                                                                                 |
| **Progress indicator** | "12 of 25 questions completed" with a progress bar at the top of the screen.                                                                                                                                                                                                          |

#### Completion Tracking

| Metric          | Description                                                                                                                               |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| % Complete      | Real-time progress within an active inspection                                                                                            |
| Time Spent      | Duration from inspection start to submission                                                                                              |
| Overdue Alerts  | If a scheduled inspection is not started within 24 hours of its due date, an alert is sent to the assigned inspector and their supervisor |
| Compliance Rate | % of scheduled inspections completed on time (visible on the Property Manager dashboard)                                                  |

---

### Recurring Task Scheduler

Transforms reactive maintenance into planned, preventive maintenance by scheduling tasks on defined frequencies with automatic assignment, escalation, and forecasting.

#### Frequency Options

| Frequency     | Detail                                          |
| ------------- | ----------------------------------------------- |
| Daily         | Every day at a configurable time                |
| Weekly        | Specific day(s) of the week                     |
| Bi-weekly     | Every 2 weeks on a specific day                 |
| Monthly       | Specific day of the month (1-28, or "last day") |
| Quarterly     | Every 3 months on a specific day                |
| Semi-annually | Every 6 months on a specific day                |
| Annually      | Specific month and day                          |
| Custom        | Every N days (range: 1-365)                     |

#### Task Template

| Field              | Type            | Max Length | Required | Default | Validation                                                    | Error Message                                     |
| ------------------ | --------------- | ---------- | -------- | ------- | ------------------------------------------------------------- | ------------------------------------------------- |
| Task Name          | Text            | 200 chars  | Yes      | --      | Non-empty                                                     | "Task name is required"                           |
| Description        | Textarea        | 2000 chars | No       | --      | --                                                            | --                                                |
| Category           | Dropdown        | --         | Yes      | --      | Must select from maintenance categories                       | "Select a category"                               |
| Frequency          | Dropdown        | --         | Yes      | --      | Must select                                                   | "Select a frequency"                              |
| Assigned To        | Dropdown        | --         | Yes      | --      | Staff member or vendor                                        | "Select an assignee"                              |
| Estimated Duration | Duration picker | --         | No       | --      | 15 min - 8 hours                                              | "Duration must be between 15 minutes and 8 hours" |
| Equipment Link     | Dropdown        | --         | No       | --      | Must be valid asset from Equipment Tracking                   | --                                                |
| Checklist Link     | Dropdown        | --         | No       | --      | Must be valid inspection checklist                            | --                                                |
| Start Date         | Date            | --         | Yes      | Today   | Cannot be in the past                                         | "Start date cannot be in the past"                |
| End Date           | Date            | --         | No       | --      | Must be after start date. If blank, task recurs indefinitely. | "End date must be after start date"               |
| Priority           | Dropdown        | --         | No       | Normal  | Low, Normal, High, Urgent                                     | --                                                |

#### Calendar View

A calendar view displays all scheduled recurring tasks with color coding by category. Tasks can be dragged to reschedule a single occurrence (the underlying frequency remains unchanged).

| Feature                | Detail                                                                                                                                                                                                       |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Views**              | Month, Week, Day, Agenda (list)                                                                                                                                                                              |
| **Color coding**       | By category: HVAC (blue), Plumbing (cyan), Electrical (yellow), Fire Safety (red), General (gray)                                                                                                            |
| **Drag to reschedule** | Dragging a task to a different day reschedules that single occurrence only. A confirmation dialog appears: "Reschedule this occurrence to [new date]? Future occurrences will follow the original schedule." |

#### Skip / Defer

| Action    | Detail                                                                                                                                                                                                     |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Skip**  | Skip a single occurrence with a required reason (dropdown: "Holiday", "Equipment Unavailable", "Staff Unavailable", "Weather", "Other" + free text). Skipped tasks are logged but do not count as overdue. |
| **Defer** | Postpone by N days (1-30). The deferred occurrence moves forward; subsequent occurrences remain on the original schedule.                                                                                  |

#### Dependencies

| Aspect                             | Detail                                                                                                                  |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **Definition**                     | Task B can be configured to depend on Task A. Task B does not appear in the work queue until Task A is marked complete. |
| **UI**                             | In the task template editor, a "Depends On" dropdown lists other recurring tasks.                                       |
| **Circular dependency prevention** | The system prevents A → B → A chains. Error: "This would create a circular dependency."                                 |

#### Escalation

| Threshold                                              | Action                                                                                     |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| Task overdue by 1x the estimated duration              | Notification to assigned staff: "Task [Name] is overdue."                                  |
| Task overdue by configured threshold (default: 2 days) | Notification to supervisor (Security Supervisor or Property Manager depending on category) |
| Task overdue by 2x configured threshold                | Notification to Property Manager + auto-priority upgrade to "Urgent"                       |

#### Forecasting

| Aspect                  | Detail                                                                                                           |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------- |
| **View**                | "Maintenance Forecast" report showing all scheduled tasks for the next 12 months                                 |
| **Resource allocation** | A heatmap showing tasks per staff member per week. Helps identify overloaded weeks and redistribute work.        |
| **Cost projection**     | If tasks have associated vendor costs or equipment parts costs, the forecast includes projected spend per month. |

---

### Vendor Compliance Dashboard

Ensures all vendors working on the property carry valid insurance and meet compliance requirements. Prevents liability exposure from uninsured or underinsured contractors.

#### 5-Status Cards

The dashboard opens with 5 status summary cards:

| Status        | Color    | Definition                                                                   | Card Shows                        |
| ------------- | -------- | ---------------------------------------------------------------------------- | --------------------------------- |
| Compliant     | Green    | All required insurance documents are current and valid                       | Count of compliant vendors        |
| Expiring Soon | Yellow   | At least one insurance document expires within 30 days                       | Count + "earliest expiry: [date]" |
| Expired       | Red      | At least one insurance document has expired                                  | Count + days since expiry         |
| Non-Compliant | Dark red | Vendor has never provided required documentation                             | Count                             |
| Not Tracked   | Gray     | Vendor's compliance tracking is disabled (e.g., one-time vendor, small jobs) | Count                             |

Clicking any card filters the vendor list below to show only vendors in that status.

#### Insurance Document Upload

Each vendor profile includes an "Insurance & Compliance" tab with document upload capability:

| Document Type          | Description                                          | Required                      |
| ---------------------- | ---------------------------------------------------- | ----------------------------- |
| General Liability      | Covers property damage and bodily injury claims      | Yes (for all tracked vendors) |
| Workers' Compensation  | Covers worker injuries on the job site               | Yes (if vendor has employees) |
| Professional Liability | Covers errors and omissions in professional services | Configurable per vendor type  |
| Auto Insurance         | Covers vehicle-related incidents                     | Configurable per vendor type  |

Each document upload captures:

| Field              | Type                                  | Required | Validation                   |
| ------------------ | ------------------------------------- | -------- | ---------------------------- |
| Document file      | File upload (PDF, JPG, PNG) max 10 MB | Yes      | Accepted formats only        |
| Policy Number      | Text (50 chars)                       | Yes      | Non-empty                    |
| Coverage Amount    | Currency                              | Yes      | Positive number              |
| Effective Date     | Date                                  | Yes      | Cannot be in the future      |
| Expiry Date        | Date                                  | Yes      | Must be after effective date |
| Insurance Provider | Text (200 chars)                      | No       | --                           |

#### Expiry Date Extraction (v2)

| Aspect                 | Detail                                                                                                                                                                                             |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Method**             | OCR (Optical Character Recognition) applied to uploaded insurance documents                                                                                                                        |
| **Behavior**           | On upload, the system attempts to extract the expiry date from the document. If found, the expiry date field is pre-filled (user can confirm or override). If not found, manual entry is required. |
| **Accuracy indicator** | Pre-filled dates show a confidence badge: "Extracted from document (verify)" in yellow. Once the user confirms, it changes to a standard field.                                                    |

#### Auto-Renewal Reminders

| Reminder        | Timing                | Channel            | Recipient                                                |
| --------------- | --------------------- | ------------------ | -------------------------------------------------------- |
| First reminder  | 60 days before expiry | Email              | Vendor contact email                                     |
| Second reminder | 30 days before expiry | Email              | Vendor contact email + Property Manager                  |
| Third reminder  | 14 days before expiry | Email + SMS        | Vendor contact email + Property Manager                  |
| Final reminder  | 7 days before expiry  | Email + SMS + Push | Vendor contact email + Property Manager + Property Admin |
