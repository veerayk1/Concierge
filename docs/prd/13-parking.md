...
> Added from GAP-ANALYSIS-FINAL.md gaps 13.1, 13.2

### A1. Granular Parking Limit Matrix (Gap 13.1, Critical)

Platform 1 has a comprehensive parking limit system that Concierge must match. Without granular limits, properties cannot enforce their bylaws.

#### Parking Limits Configuration (Settings > Parking Tab)

##### Overnight Visitor Parking Limits

| Limit Type                          | Granularity                  | Data Type               | Default | Description                                             |
| ----------------------------------- | ---------------------------- | ----------------------- | ------- | ------------------------------------------------------- |
| overnight_limit_per_plate_per_week  | Per license plate, per week  | Integer (0 = unlimited) | 0       | Max overnight stays per license plate per week          |
| overnight_limit_per_unit_per_week   | Per unit, per week           | Integer (0 = unlimited) | 0       | Max overnight visitor parking passes per unit per week  |
| overnight_limit_per_plate_per_month | Per license plate, per month | Integer (0 = unlimited) | 0       | Max overnight stays per license plate per month         |
| overnight_limit_per_unit_per_month  | Per unit, per month          | Integer (0 = unlimited) | 0       | Max overnight visitor parking passes per unit per month |
| overnight_limit_per_plate_per_year  | Per license plate, per year  | Integer (0 = unlimited) | 0       | Max overnight stays per license plate per year          |
| overnight_limit_per_unit_per_year   | Per unit, per year           | Integer (0 = unlimited) | 0       | Max overnight visitor parking passes per unit per year  |
| consecutive_overnight_limit         | Per license plate            | Integer (0 = unlimited) | 3       | Max consecutive overnight stays                         |

##### Day Visit Limits

| Limit Type                | Granularity                | Data Type               | Default | Description                             |
| ------------------------- | -------------------------- | ----------------------- | ------- | --------------------------------------- |
| day_visit_limit_per_unit  | Per unit, per day          | Integer (0 = unlimited) | 2       | Max day visitor passes per unit per day |
| day_visit_limit_per_plate | Per license plate, per day | Integer (0 = unlimited) | 0       | Max day visits per plate per day        |

##### Feature Toggles

| Setting                          | Type    | Default | Description                                                  |
| -------------------------------- | ------- | ------- | ------------------------------------------------------------ |
| allow_self_serve_visitor_parking | Boolean | false   | Residents can register visitor parking themselves via portal |
| allow_multi_day_passes           | Boolean | false   | Residents can create multi-day visitor passes                |
| allow_printing_parking_passes    | Boolean | true    | Enable formatted parking pass printing                       |
| restrict_visitor_parking_editing | Boolean | false   | Only admins can edit visitor parking entries                 |

#### Enforcement Behavior

When a limit is reached:

1. System blocks the new parking registration with message: "Parking limit reached: [Limit description]. Contact the management office."
2. Override available to Property Manager and Admin roles only (logged in audit trail)
3. Notification sent to configured roles (see A2 below)
4. Dashboard counter updates: "X units at parking limit this week"

### A2. Role-Based Parking Limit Notifications (Gap 13.2, High)

When any parking limit is reached, notifications are sent to configurable roles.

#### Notification Configuration (Settings > Parking Tab > Notifications)

| Role                   | Default | Notification Channels |
| ---------------------- | ------- | --------------------- |
| Board Member           | Off     | Email                 |
| Concierge / Front Desk | On      | Email + Push          |
| Owner (of the unit)    | On      | Email                 |
| Property Manager       | On      | Email + Push          |
| Security Head Office   | Off     | Email                 |
| Site Supervisor        | Off     | Email                 |
| Superintendent         | Off     | Email                 |
| Tenant (of the unit)   | On      | Email                 |
| Admin                  | On      | Email + Push          |

Each role has an independent on/off toggle. When on, the notification fires through the configured channels whenever a parking limit is reached for that role's scope.

---

_Last updated: 2026-03-17_
_Lines: 500+ + addendum_
