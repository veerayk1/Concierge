...
### Tab: Branding & White-Label (Settings > Branding)

Allows Property Admins to customize the visual identity of their property within the platform. Branding changes are reflected on the login page, sidebar, email templates, and the vanity URL landing page.

**Access**: Property Admin, Super Admin

#### Configuration Fields

| Field                  | Type                  | Max Length | Required | Default                      | Validation                                                                                                                                                                                                | Error Message                                                                                                     |
| ---------------------- | --------------------- | ---------- | -------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| Property Logo          | File upload (PNG/SVG) | 2 MB       | No       | Concierge default logo       | Accepted formats: PNG, SVG. Max dimensions: 400x400px. Min dimensions: 64x64px.                                                                                                                           | "Upload a PNG or SVG file under 2 MB (64x64 to 400x400 pixels)"                                                   |
| Primary Brand Color    | Color picker (hex)    | 7 chars    | No       | `#2563EB` (Concierge blue)   | Valid hex color. Must meet WCAG 2.2 AA contrast ratio (4.5:1) against white background for text usage.                                                                                                    | "Selected color does not meet accessibility contrast requirements. Choose a darker shade."                        |
| Vanity URL Slug        | Text                  | 30 chars   | No       | --                           | Lowercase letters, numbers, and hyphens only. Must start with a letter. Must be unique across all properties. No reserved words (login, admin, api, app, help, support, status, blog, pricing, features). | "This URL is already taken" / "Only lowercase letters, numbers, and hyphens allowed" / "Must start with a letter" |
| Welcome Message        | Text                  | 200 chars  | No       | "Welcome to [Property Name]" | Non-empty if provided. No HTML.                                                                                                                                                                           | "Welcome message cannot exceed 200 characters"                                                                    |
| Custom Favicon         | File upload (ICO/PNG) | 100 KB     | No       | Concierge default favicon    | Accepted formats: ICO, PNG. Dimensions: 32x32 or 64x64 pixels.                                                                                                                                            | "Upload an ICO or PNG file, 32x32 or 64x64 pixels, under 100 KB"                                                  |
| Email Header Logo      | File upload (PNG)     | 1 MB       | No       | Same as Property Logo        | Accepted format: PNG. Max width: 600px (for email client compatibility).                                                                                                                                  | "Upload a PNG file, max 600px wide, under 1 MB"                                                                   |
| Email Footer Text      | Text                  | 500 chars  | No       | "[Property Name]             | [Address]"                                                                                                                                                                                                | No HTML. Supports merge fields: `{{property_name}}`, `{{property_address}}`, `{{property_phone}}`.                | "Footer text cannot exceed 500 characters" |
| Email Reply-To Address | Email                 | 254 chars  | No       | `noreply@concierge.com`      | Valid email format. Domain must be verified (SPF/DKIM check on save).                                                                                                                                     | "Enter a valid email address" / "This email domain has not been verified"                                         |

#### Preview Panel

A live preview panel occupies the right side of the Branding tab. It shows a mockup of the login page with the current branding applied: logo at the top, welcome message, primary color on the "Sign In" button, custom favicon in the browser tab simulation. Changes update the preview in real-time before saving.

#### Save Branding Button

| State       | Behavior                                                                                                       |
| ----------- | -------------------------------------------------------------------------------------------------------------- |
| **Default** | "Save Branding" -- blue filled button. Disabled until at least one field is modified.                          |
| **Loading** | Spinner + "Saving..."                                                                                          |
| **Success** | Toast: "Branding updated successfully. Changes are live." + CDN cache purge triggered for the vanity URL page. |
| **Failure** | Toast (red): "Failed to save branding. Please try again."                                                      |

> **Tooltip -- Vanity URL**: "Create a custom URL for your property's login page. Residents can bookmark concierge.com/your-building to access the login page directly with your branding."

> **Tooltip -- Primary Brand Color**: "This color is used for primary buttons, active navigation items, and accent elements throughout the platform. It must meet accessibility contrast standards."

---

### Tab: Demo Management (Settings > Demo)

**Access**: Super Admin only

Provides tools for creating, managing, and monitoring demo and training environments. Demo properties are used for sales presentations and prospect walkthroughs. Training properties are used for onboarding new staff.

#### Create Demo from Template

| Field         | Type             | Required | Default                      | Validation                                      | Error Message                           |
| ------------- | ---------------- | -------- | ---------------------------- | ----------------------------------------------- | --------------------------------------- |
| Demo Name     | Text (100 chars) | Yes      | "Demo - [Date]"              | Non-empty, unique across active demos           | "A demo with this name already exists"  |
| Template      | Dropdown         | Yes      | "Standard Condo (200 units)" | Must select from available templates            | "Select a template"                     |
| Expiry (days) | Number           | No       | 30                           | 1-365                                           | "Expiry must be between 1 and 365 days" |
| Assigned To   | User dropdown    | No       | Current user                 | Must be an active Super Admin or Property Admin | "Select a valid user"                   |

**Available templates**: Standard Condo (200 units), Luxury Tower (500 units), Small Building (50 units), Multi-Property Portfolio (3 properties). Templates contain pre-generated: units, sample residents (with fictional data), sample events, sample packages, sample maintenance requests, sample amenity bookings, and sample announcements.

**Create Demo Button**: "Create Demo" -- generates the demo property with seed data. Estimated creation time: 5-15 seconds. Progress indicator shown.
...
| Created By    | Admin who created the demo                                                                                           |
| Template      | Which template was used                                                                                              |
| Last Accessed | Timestamp of most recent login to this demo                                                                          |
| Expires       | Date the demo auto-deactivates                                                                                       |
| Status        | Active, Expired, Deactivated                                                                                         |
| Actions       | "Reset" (re-seed data to template defaults), "Extend" (add 30 days), "Delete" (permanent removal after confirmation) |

#### Training Environments

| Field                 | Description                                                                            |
| --------------------- | -------------------------------------------------------------------------------------- |
| Active Training Count | Number of active training properties                                                   |
| Create New            | "Create Training Environment" button -- same template selection as demos but no expiry |
| View Progress         | Links to Training/LMS module (PRD 12) filtered to the training property                |

#### Demo Data Templates

| Action          | Description                                                                                                                                                           |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| View Templates  | List all available seed data templates                                                                                                                                |
| Create Template | "Create from Production" -- clone a production property's configuration (event types, categories, amenities, custom fields) without resident data into a new template |
| Edit Template   | Modify template metadata (name, description, unit count)                                                                                                              |
| Delete Template | Remove template (only if not used by active demos)                                                                                                                    |

---

### Tab: Backup & Recovery (Settings > Backup)

**Access**: Super Admin only

Provides visibility into the automated backup system and manual backup/restore controls. Backups are critical for disaster recovery and compliance (see CLAUDE.md: "Data loss = bankruptcy").

#### Backup Status Dashboard

| Field                 | Description                                                                                                                                                |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Last Backup           | Timestamp of the most recent successful backup (e.g., "March 16, 2026, 3:00 AM EST")                                                                       |
| Backup Status         | Badge: "Healthy" (green), "Warning" (yellow -- last backup older than 36 hours), "Critical" (red -- last backup older than 48 hours or last backup failed) |
| Backup Schedule       | Current schedule display (e.g., "Daily at 3:00 AM EST")                                                                                                    |
| Next Scheduled Backup | Timestamp of the next automatic backup                                                                                                                     |

#### Schedule Configuration

| Field            | Type        | Required | Default | Validation                    | Error Message               |
| ---------------- | ----------- | -------- | ------- | ----------------------------- | --------------------------- |
| Backup Time      | Time picker | Yes      | 03:00   | Valid time, property timezone | "Select a backup time"      |
| Frequency        | Dropdown    | Yes      | Daily   | Daily, Twice Daily, Weekly    | "Select a frequency"        |
| Retention Period | Dropdown    | Yes      | 30 days | 7, 14, 30, 60, 90 days        | "Select a retention period" |

#### Manual Backup

| Aspect           | Detail                                                                                                   |
| ---------------- | -------------------------------------------------------------------------------------------------------- |
| **Button**       | "Create Backup Now" -- blue outlined button                                                              |
| **Behavior**     | Triggers an immediate backup. Progress bar shown. Estimated time: 1-10 minutes depending on data volume. |
| **Rate limit**   | Maximum 3 manual backups per day per property.                                                           |
| **Confirmation** | Toast on completion: "Backup completed successfully. Size: [X] MB."                                      |

#### Backup History

| Column   | Description                                                                                     |
| -------- | ----------------------------------------------------------------------------------------------- |
| Date     | Timestamp of backup creation                                                                    |
| Size     | Backup file size in MB                                                                          |
| Status   | Completed (green), Failed (red), In Progress (blue spinner)                                     |
| Type     | Automatic, Manual                                                                               |
| Download | Download link (signed URL, valid for 1 hour). Requires 2FA confirmation before generating link. |

Displays the last 30 days of backups, paginated (10 per page).
...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Access**       | Super Admin only                                                                                                                                                                                                                                                                                           |
| **Flow**         | Select a backup from the history list > Click "Restore" > Confirmation dialog: "Restoring this backup will replace all current data for [Property Name] with the data from [Backup Date]. This action cannot be undone. Type the property name to confirm." > Requires 2FA confirmation. > Restore begins. |
| **Duration**     | Estimated 5-30 minutes depending on data volume. A maintenance mode banner is shown to all users of the property during restore: "System maintenance in progress. Please wait."                                                                                                                            |
| **Notification** | On completion, email notification sent to all Property Admins: "Data restore completed for [Property Name] from backup dated [Date]."                                                                                                                                                                      |

#### Backup Scope

| Included                                                                                          | Excluded                                             |
| ------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| All property data (units, residents, events, packages, maintenance, bookings, announcements)      | Audit logs (separate retention policy, see Tab 10)   |
