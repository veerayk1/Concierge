...
Stores operations created while the device is offline. Maximum queue size: 1,000 operations per device. When the queue reaches 1,000 items, the oldest non-critical operations (create_note, update_status) are discarded to make room for new ones. Critical operations (create_event with incident type, create_request) are never discarded. If the queue is full and the new operation is non-critical, the user sees: "Offline storage is full. Connect to the internet to sync pending changes before creating new items."
...
> Added from GAP-ANALYSIS-FINAL.md gap 17.1

### A1. NFC/QR Badge Scanning for Resident Identification (Gap 17.1, Medium)

Industry research revealed that one platform supports user badge scanning through a mobile app, with a dedicated "User Badge Scans" report tracking scan activity. Concierge should support this capability for staff using mobile devices to verify resident identity.

#### Feature Scope (v2)

| Aspect             | Specification                                                                                                                                                                          |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Technology**     | NFC (primary) and QR code (fallback). The PWA uses the Web NFC API (Chrome Android only for v2) and the device camera for QR scanning.                                                 |
| **Use case**       | Security guards and concierge staff scan a resident's digital badge (displayed on the resident's phone or a printed card) to verify identity, log entry, or confirm package pickup.    |
| **Badge content**  | QR code or NFC tag encodes a signed, time-limited token: `{user_id, property_id, issued_at, expires_at, signature}`. Token validity: 24 hours.                                         |
| **Scan result**    | On successful scan, the staff device shows: resident name, unit number, photo (if on file), and account status (active/inactive/suspended). A "Log Entry" quick action button appears. |
| **Failed scan**    | If the token is expired, invalid, or for a different property: "Badge not recognized. Ask the resident to refresh their badge in the app, or look them up manually."                   |
| **iOS limitation** | Web NFC is not available on iOS Safari. iOS users must use QR code scanning via the camera. The UI auto-detects the platform and shows the appropriate scan option.                    |

#### Badge Scan Report

A "Badge Scans" report is added to the Security report category in PRD 10:

| Attribute           | Detail                                                                                                                        |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **Default columns** | Scan Date/Time, Staff Name, Resident Name, Unit, Scan Method (NFC / QR), Result (Success / Failed), Location (if GPS enabled) |
| **Parameters**      | Date Range, Staff filter, Result filter                                                                                       |
| **Access**          | Property Manager, Security Manager, Admin                                                                                     |

#### Resident Badge Generation

Residents access their digital badge from the resident portal under My Account > Digital Badge. The badge displays a QR code and, on NFC-enabled Android devices, can be tapped against a staff device. The badge refreshes its token automatically every 24 hours.
