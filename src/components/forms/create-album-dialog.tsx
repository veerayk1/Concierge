...
/**
 * Create Album Dialog — New Photo Album
 */
...
export function CreateAlbumDialog({
  ...
  async function onSubmit(data: AlbumInput) {
    setServerError(null);
    try {
      const response = await fetch('/api/v1/photo-albums', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        setServerError(result.message || 'Failed to create album');
        return;
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-xl overflow-y-auto">
        ...
        <DialogDescription className="text-[14px] text-neutral-500">
          Create a new photo album for building or community photos.
        </DialogDescription>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 flex flex-col gap-5" noValidate>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[14px] font-medium text-neutral-700">
                Visibility<span className="text-error-500 ml-0.5">*</span>
              </label>
              <select
                {...register('visibility')}
                className={errors.visibility ? selectErrorClass : selectClass}
              >
