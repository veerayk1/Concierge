...
/**
 * Create Forum Thread Dialog — per PRD Discussion Forum
 */
...
type ForumThreadInput = z.infer<typeof forumThreadSchema>;
...
export function CreateForumThreadDialog({
  ...
  } = useForm<ForumThreadInput>({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: zodResolver(forumThreadSchema) as any,
    defaultValues: {
      title: '',
      category: '',
      body: '',
  ...
  async function onSubmit(data: ForumThreadInput) {
    setServerError(null);
    try {
      ...
      if (!response.ok) {
        ...
        setServerError(result.message || 'Failed to create thread');
        return;
      }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 flex flex-col gap-5" noValidate>
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea
              ...
              rows={5}
              className={`${textareaBase} ${errors.body ? textareaError : textareaDefault}`}
              maxLength={5000}
            />
            {errors.body && (
