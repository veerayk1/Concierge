...
/**
 * Create Help Ticket Dialog — per PRD 25 Help Center
 */
...
const helpTicketSchema = z.object({
  ...
  description: z.string().min(10, 'Description is required (at least 10 characters)').max(5000),
  category: z.string().min(1, 'Select a category'),
  priority: z.string().min(1, 'Select a priority'),
});

type HelpTicketInput = z.infer<typeof helpTicketSchema>;
...
interface CreateHelpTicketDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

export function CreateHelpTicketDialog({
  open,
  onOpenChange,
  onSuccess,
}: CreateHelpTicketDialogProps) {
  ...
  } = useForm<HelpTicketInput>({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: zodResolver(helpTicketSchema) as any,
    defaultValues: {
      subject: '',
      description: '',
      category: '',
  ...
  async function onSubmit(data: HelpTicketInput) {
    setServerError(null);
    try {
      const response = await fetch('/api/v1/help/tickets', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        setServerError(result.message || 'Failed to submit ticket');
        return;
      }
