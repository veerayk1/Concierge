  ...
  useEffect(() => {
    ...
    async function init(): Promise<(() => void) | undefined> {
      try {
        ...
        // failIfMajorPerformanceCaveat: false avoids hard-failing on SwiftShader /
        // some corporate GPUs; if WebGL still cannot start, catch below and bail
        // so the rest of the marketing page (CSS hero fallback) still renders.
        ...
        // WebGL unavailable or context creation failed — HeroSection CSS fallback
        // still shows; never throw (avoids blank page / unhandled rejection).
        return undefined;
      }
    }
    ...
    init()
      ...
      .catch(() => {
        /* init() already catches; defensive */
      });
