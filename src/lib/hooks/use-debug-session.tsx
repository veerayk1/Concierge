...
interface DebugSessionContextValue {
  ...
  reportEvent: (
    event: Omit<DebugEventPayload, 'sessionId' | 'actionTrail' | 'environment'>,
  ) => void;
  isEnabled: boolean;
}
...
export function DebugSessionProvider({ children }: DebugSessionProviderProps) {
  const isEnabled = typeof window !== 'undefined' && process.env['NODE_ENV'] !== 'production';
  ...
  const pushAction = useCallback(
    (action: Omit<ActionEntry, 'timestamp' | 'route'>): void => {
      if (!isEnabled) return;
  ...
  useEffect(() => {
    ...
    setGlobalDebugReporter((event) => {
      ...
      const timeout = setTimeout(() => controller.abort(), 5000);
      const authHeader = getAccessToken();
      const demoRoleGlobal =
        typeof window !== 'undefined' ? localStorage.getItem('demo_role') : null;
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      if (demoRoleGlobal) {
  ...
  return (
    <DebugSessionContext.Provider value={contextValue}>{children}</DebugSessionContext.Provider>
  );
}
