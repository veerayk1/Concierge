...
export default function ShiftLogEntryDetailPage() {
  ...
  useEffect(() => {
    async function fetchEntry() {
      try {
        setLoading(true);
        const res = await apiRequest(`/api/v1/shift-log/${id}`, { method: 'GET' });
        if (res.status === 404) {
          setNotFound(true);
          return;
        }
            ...
            <CardContent>
              ...
              <div className="mt-4 flex gap-2">
                <input
                  ...
                  disabled
                  title="Shift log entries are append-only. Threaded comments are coming in the next release."
                ...
                <Button
                  size="sm"
                  disabled
                  title="Shift log comments are coming in the next release."
                >
                  Post
                </Button>
              </div>
