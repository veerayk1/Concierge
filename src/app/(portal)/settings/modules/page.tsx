  ...
  return (
    <div
      className={`
        ...
        ${enabled ? 'border-neutral-200 shadow-sm' : 'border-neutral-100 bg-neutral-50/50 opacity-70'}
      `}
    >
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3.5">
          <div
            className={`
              ...
              ${enabled ? 'bg-primary-50' : 'bg-neutral-100'}
            `}
          >
            <Icon
              className={`h-5 w-5 ${enabled ? 'text-primary-500' : 'text-neutral-400'}`}
            />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${tierColor.bg} ${tierColor.text}`}
              >
        ...
        <div className="ml-4 shrink-0">
          <Toggle enabled={enabled} onChange={(val) => onToggle(moduleKey, val)} disabled={toggling} />
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col gap-10">
        {Object.entries(grouped).map(([category, modules]) => (
          <section key={category}>
            <h2 className="mb-4 text-[12px] font-semibold uppercase tracking-[0.08em] text-neutral-400">
