...
export default function ComplianceFrameworkDetailPage() {
  ...
  useEffect(() => {
    async function fetchFramework() {
      try {
        setLoading(true);
        const res = await apiRequest(`/api/v1/compliance/${id}`, { method: 'GET' });
        if (res.status === 404) {
          setNotFound(true);
          return;
        }
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
        ...
        <h1 className="text-[24px] font-bold text-neutral-900">Framework not found</h1>
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
        ...
        <h1 className="text-[24px] font-bold text-neutral-900">Error loading framework</h1>
