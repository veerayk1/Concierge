/**
 * Multi-Property Management — Comprehensive TDD Tests
 *
 * Covers all 14 test scenarios for management companies managing
 * multiple properties from a single account.
 */
...
const ROLE_CONCIERGE_ID = 'role-concierge-id';
...
// =========================================================================
// 1. GET /properties — lists all properties for the authenticated management company
// =========================================================================

describe('GET /properties — list properties', () => {
...
// =========================================================================
// 2. POST /properties — create new property
// =========================================================================
...
// =========================================================================
// 3. PATCH /properties/:id — update property details
// =========================================================================
...
// =========================================================================
// 5. Cross-property dashboard: aggregate KPIs
// =========================================================================

describe('Cross-property dashboard — aggregate KPIs', () => {
...
// =========================================================================
// 7. Property comparison: side-by-side metrics
// =========================================================================

describe('Property comparison — side-by-side metrics', () => {
  it('returns comparison metrics for selected properties', async () => {
    mockPropertyFindMany.mockResolvedValue([sampleProperty(), samplePropertyB]);
...
// =========================================================================
// 8. User assignment: assign staff to specific properties
// =========================================================================
...
// =========================================================================
// 9. Staff can be assigned to multiple properties
// =========================================================================
...
// =========================================================================
// 10. Property settings are independent
// =========================================================================
...
describe('Property settings independence', () => {
  ...
  it('changing settings on one property does not affect another', async () => {
    mockPropertySettingsUpsert.mockResolvedValue({
      id: 'ps-1',
      propertyId: PROPERTY_A_ID,
      brandingConfig: { primaryColor: '#FF0000' },
    });
    mockPropertySettingsFindUnique.mockResolvedValue({
      id: 'ps-2',
      propertyId: PROPERTY_B_ID,
    ...
    const { PATCH: updateSettings } = await import('../[id]/settings/route');
    const req = createPatchRequest(
      `/api/v1/properties/${PROPERTY_A_ID}/settings`,
      { brandingConfig: { primaryColor: '#FF0000' } },
      { headers: { 'x-demo-role': 'super_admin' } },
    );
    const res = await updateSettings(req, { params: Promise.resolve({ id: PROPERTY_A_ID }) });

    expect(res.status).toBe(200);

    // Verify property B settings are unchanged
    const { GET: getSettings } = await import('../[id]/settings/route');
    const req2 = createGetRequest(`/api/v1/properties/${PROPERTY_B_ID}/settings`, {
      headers: { 'x-demo-role': 'super_admin' },
    });
    const res2 = await getSettings(req2, { params: Promise.resolve({ id: PROPERTY_B_ID }) });
    const body2 = await parseResponse<{ data: Record<string, unknown> }>(res2);

    expect(res2.status).toBe(200);
    expect(body2.data.settings).toEqual(expect.objectContaining({ brandingConfig: null }));
  });
});

// =========================================================================
// 11. Billing: per-property subscription tracking
// =========================================================================
...
// =========================================================================
// 12. Property deactivation: soft-disable with data retention
// =========================================================================

describe('Property deactivation — soft-disable', () => {
  it('deactivates a property without deleting data', async () => {
    ...
    // Data is retained (not hard-deleted)
...
// =========================================================================
// 13. White-label: per-property branding
// =========================================================================
...
describe('White-label — per-property branding', () => {
  ...
  it('each property has independent branding', async () => {
    const brandingA = { logo: 'logo-a.png', primaryColor: '#111' };
    const brandingB = { logo: 'logo-b.png', primaryColor: '#222' };

    // Property A has branding set
    mockPropertyFindUnique.mockResolvedValueOnce(sampleProperty({ branding: brandingA }));
    const req1 = createGetRequest(`/api/v1/properties/${PROPERTY_A_ID}`, {
      headers: { 'x-demo-role': 'super_admin' },
    });
    const res1 = await GET_BY_ID(req1, { params: Promise.resolve({ id: PROPERTY_A_ID }) });
    const body1 = await parseResponse<{ data: Record<string, unknown> }>(res1);

    // Property B has different branding
    mockPropertyFindUnique.mockResolvedValueOnce(samplePropertyB);
    const req2 = createGetRequest(`/api/v1/properties/${PROPERTY_B_ID}`, {
      headers: { 'x-demo-role': 'super_admin' },
    });
    const res2 = await GET_BY_ID(req2, { params: Promise.resolve({ id: PROPERTY_B_ID }) });
    const body2 = await parseResponse<{ data: Record<string, unknown> }>(res2);

    expect(body1.data.branding).toEqual(brandingA);
    expect(body2.data.branding).not.toEqual(brandingA);
  });
});

// =========================================================================
// 14. Vanity URL: per-property login URL
// =========================================================================

describe('Vanity URL — per-property login URL', () => {
  ...
  it('sets a unique propertyCode for login routing', async () => {
    mockPropertyFindUnique.mockResolvedValue(sampleProperty());
    mockPropertyUpdate.mockResolvedValue(sampleProperty({ propertyCode: 'MPL-NEW' }));

    const req = createPatchRequest(
      `/api/v1/properties/${PROPERTY_A_ID}`,
      { propertyCode: 'MPL-NEW' },
      { headers: { 'x-demo-role': 'super_admin' } },
    );
    const res = await PATCH(req, { params: Promise.resolve({ id: PROPERTY_A_ID }) });
    const body = await parseResponse<{ data: Record<string, unknown> }>(res);
    ...
    expect(res.status).toBe(200);
    expect(body.data.propertyCode).toBe('MPL-NEW');
  });
