...
export async function createDebugEvent(input: CreateDebugEventInput): Promise<string | null> {
  ...
  try {
    ...
    const severity: DebugSeverity = input.severity ?? autoSeverity(input.type, input.errorCode);

    // Sanitize context JSONB
    const safeContext = input.context
      ? (sanitizeForLog(input.context) as Record<string, unknown>)
      : null;

    // Stack trace only in non-production
    const stackTrace = process.env['NODE_ENV'] !== 'production' ? (input.stackTrace ?? null) : null;
...
export async function updateDebugEvent(
  id: string,
  updates: DebugEventUpdates,
): Promise<DebugEvent> {
...
const MODULE_FILE_MAP: Record<string, ModuleFiles> = {
  ...
  maintenance: {
    frontend: [
      'src/app/(portal)/maintenance/page.tsx',
      'src/app/(portal)/maintenance/[id]/page.tsx',
    ],
  ...
  auth: {
    ...
    backend: [
      ...
      'src/server/middleware/chain.ts',
    ],
  ...
  billing: {
    frontend: [
      'src/app/(portal)/settings/billing/page.tsx',
      'src/app/(portal)/system/billing/page.tsx',
    ],
  ...
  'data-migration': {
    ...
    backend: [
      'src/app/api/v1/data-migration/import/route.ts',
      'src/app/api/v1/data-migration/export/route.ts',
    ],
  ...
  developer: {
    ...
    backend: [
      'src/app/api/v1/developer/api-keys/route.ts',
      'src/app/api/v1/developer/webhooks/route.ts',
    ],
